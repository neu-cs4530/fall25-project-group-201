...
const toggleMediaPermission = async (qid: string, username: string, token: string): Promise<boolean> => {
  const data = { qid, username };
  const res = await api.post(`${QUESTION_API_URL}/toggleMediaPermission`,
    data,
    {
    headers: {
