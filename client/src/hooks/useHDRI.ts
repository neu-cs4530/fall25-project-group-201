...
const useHDRI = ({
    ...
    const loadPreset = useCallback(
        async (presetName: string): Promise<THREE.Texture> => {
            if (!sceneRef.current || !rendererRef.current || !pmremGeneratorRef.current || !loaderRef.current) {
            ...
            return new Promise((resolve, reject) => {
                loaderRef.current!.load(
                    ...
                    (texture) => {
                        ...
                        const renderTarget =
                            pmremGeneratorRef.current!.fromEquirectangular(texture);
    ...
    const switchPreset = useCallback(
        ...
        if (presetName === 'default') {
            ...
            try {
                ...
                const errorMessage =
                    err instanceof Error ? err.message : 'Unknown error loading HDRI';
    ...
    const preloadAll = useCallback(async () => {
        ...
        try {
            ...
            const errorMessage =
                err instanceof Error ? err.message : 'Unknown error preloading HDRIs';
            setError(errorMessage);
            setIsLoading(false);
